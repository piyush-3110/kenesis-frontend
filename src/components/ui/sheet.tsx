...
>(({ className, children, side = "bottom", ...props }, ref) => {
  ...
  return (
    <DialogPrimitive.Portal>
      <DialogPrimitive.Overlay className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm opacity-0 data-[state=open]:opacity-100 data-[state=closed]:opacity-0 transition-opacity duration-300 ease-out" />
      <DialogPrimitive.Content
        ref={ref}
        className={clsx(
          // Base styles
          "fixed z-50 flex flex-col gap-4 bg-[#0F0B24]/95 text-white p-6 shadow-xl border border-white/10 will-change-transform",
