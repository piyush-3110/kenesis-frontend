...
import { DASHBOARD_COLORS, DASHBOARD_TYPOGRAPHY } from '../../dashboard/constants';
import { 
  ArrowLeft, 
  FileText, 
  Download, 
...
const LearningPage: React.FC = () => {
  ...
  const loadModulesForChapters = async (chapters: Chapter[]): Promise<Chapter[]> => {
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      try {
        console.log(`📚 [MODULES] Loading modules for chapter: ${chapter.id} (${i + 1}/${chapters.length})`);
        ...
        const modulesResponse = await CourseAPI.getModulesForChapter(chapter.id, {
          sortBy: 'order',
          sortOrder: 'asc'
        });
        
        console.log(`📚 [MODULES] Modules response for chapter ${chapter.id}:`, JSON.stringify(modulesResponse, null, 2));
        
        if (modulesResponse.success) {
          const modules = modulesResponse.data?.modules || [];
          console.log(`📚 [MODULES] Found ${modules.length} modules for chapter ${chapter.id}`);
          chaptersWithModules.push({
            ...chapter,
            modules: modules.sort((a: Module, b: Module) => a.order - b.order)
          });
        } else {
          console.error(`❌ [MODULES] Failed to load modules for chapter ${chapter.id}:`, modulesResponse.message);
          
          // Handle rate limiting and other errors with toast notifications
          if (modulesResponse.message?.toLowerCase().includes('too many requests')) {
            ...
            console.log('⚠️ [MODULES] Rate limit detected, skipping remaining chapters');
        ...
        console.error(`❌ [MODULES] Exception loading modules for chapter ${chapter.id}:`, err);
        ...
        if (errorMessage.toLowerCase().includes('network') || errorMessage.toLowerCase().includes('fetch')) {
          addToast({
            type: 'error',
            message: 'Network error while loading modules. Please check your connection.'
          });
        } else if (errorMessage.toLowerCase().includes('timeout')) {
          addToast({
            type: 'warning',
            message: 'Request timed out while loading modules. Please try again.'
          });
        } else {
          addToast({
            type: 'error',
    ...
    if (hasRateLimitError) {
      addToast({
        type: 'warning',
        message: 'Rate limit reached. Some modules may not load. Please wait a moment and refresh the page.'
      });
    }
    ...
    console.log('📚 [MODULES] All chapters with modules loaded:', JSON.stringify(chaptersWithModules, null, 2));
  ...
  const checkAccessAndLoadCourse = async () => {
    try {
      ...
      console.log('🔐 [LEARN] Starting access check and course load...');
      console.log('🔐 [LEARN] Course ID:', courseId);

      // Check if user has access to this course
      console.log('🔐 [LEARN] Checking course access...');
      const accessResponse = await CourseAPI.checkCourseAccess(courseId);
      
      console.log('🔐 [LEARN] Access check response:', JSON.stringify(accessResponse, null, 2));
      console.log('🔐 [LEARN] Access response success:', accessResponse.success);
      console.log('🔐 [LEARN] Has access:', accessResponse.data?.hasAccess);
      
      if (accessResponse.success && accessResponse.data?.hasAccess) {
        console.log('✅ [LEARN] Access granted, loading course...');
        setHasAccess(true);
        
        // Load course data with chapters and modules
        console.log('📚 [LEARN] Loading course basic data...');
        const courseResponse = await CourseAPI.getCourse(courseId);
        
        console.log('📚 [LEARN] Course response:', JSON.stringify(courseResponse, null, 2));
        ...
        if (courseResponse.success) {
          ...
          console.log('📚 [LEARN] Course data extracted:', JSON.stringify(courseData, null, 2));
          ...
          console.log('📖 [LEARN] Chapters response:', JSON.stringify(chaptersResponse, null, 2));
          console.log('📖 [LEARN] Chapters response success:', chaptersResponse.success);
          
          if (chaptersResponse.success) {
            const chapters = chaptersResponse.data?.chapters || [];
            console.log('📖 [LEARN] Chapters extracted:', JSON.stringify(chapters, null, 2));
            ...
            console.log('📖 [LEARN] Sorted chapters:', JSON.stringify(sortedChapters, null, 2));
            
            // Load modules for each chapter
            const chaptersWithModules = await loadModulesForChapters(sortedChapters);
            ...
            console.log('📚 [LEARN] Full course object created:', JSON.stringify(fullCourse, null, 2));
            setCourse(fullCourse);
            
            // Initialize expanded chapters
            const allChapterIds = new Set<string>(chaptersWithModules.map((ch: Chapter) => ch.id));
            ...
            if (chaptersWithModules.length > 0 && chaptersWithModules[0].modules?.length > 0) {
              const firstModule = chaptersWithModules[0].modules.sort((a: Module, b: Module) => a.order - b.order)[0];
              console.log('🎯 [LEARN] Auto-selecting first module:', JSON.stringify(firstModule, null, 2));
              setSelectedModule(firstModule);
            } else {
              console.log('⚠️ [LEARN] No modules found to auto-select');
            }
          } else {
            console.error('❌ [LEARN] Failed to load chapters:', chaptersResponse.message);
            throw new Error(chaptersResponse.message || 'Failed to load chapters');
          }
        } else {
      ...
      setLoading(false);
      console.log('🔐 [LEARN] checkAccessAndLoadCourse completed');
    }
  };
  ...
  const loadModuleContent = async () => {
    ...
    try {
      ...
      console.log('📖 [MODULE] Module details:', JSON.stringify(selectedModule, null, 2));
      
      const response = await CourseAPI.getModuleContent(courseId, selectedModule.id);
      
      console.log('📖 [MODULE] Module content response:', JSON.stringify(response, null, 2));
      console.log('📖 [MODULE] Response success:', response.success);
      console.log('📖 [MODULE] Response data:', JSON.stringify(response.data, null, 2));
      ...
      if (response.success) {
        ...
        const transformedContent = {
          ...
          attachments: response.data?.attachments?.map((att: { id: string; name: string; url: string; type: string; size: number }) => ({
            id: att.id,
        ...
        console.error('❌ [MODULE] Failed to load module content:', response.message);
  ...
  const retryFailedChapters = async () => {
    ...
    const failedChaptersList = course.chapters.filter(ch => failedChapters.has(ch.id));
  ...
  const calculateProgress = (): number => {
    ...
    return allModules.length > 0 ? (completedModules.length / allModules.length) * 100 : 0;
  };
  ...
  return (
    <div className="min-h-screen flex flex-col pt-16 lg:pt-28 relative">
      ...
      <div 
        ...
        style={{ 
          background: 'linear-gradient(152.97deg, rgba(6,128,255,0.08) 18.75%, rgba(0,0,0,0.4) 100%)',
      ...
      >
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
            ...
            <div 
              className="h-5 w-px"
              style={{ background: 'linear-gradient(180deg, #0680FF 0%, transparent 100%)' }}
            />
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                {course.level && (
                  <span 
                    className={`px-2 py-0.5 text-xs rounded-full border ${getBadgeColor(course.level)}`}
                  >
                    {course.level}
                  </span>
                )}
            ...
            <div className="hidden sm:flex items-center gap-3">
              <div className="text-right">
                ...
                <div className="text-xs text-gray-400">
                  {getAllModules().filter(m => m.completed).length}/{getAllModules().length} lessons
                </div>
              </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`${sidebarOpen ? 'w-72 sm:w-80' : 'w-0'} transition-all duration-300 overflow-hidden flex-shrink-0 relative`}>
          ...
          >
            ...
            <div 
              ...
              style={{ background: 'linear-gradient(180deg, #0680FF 0%, rgba(1,5,25,0.5) 88.45%)' }}
            />
            ...
            <div className="h-full overflow-y-auto backdrop-blur-sm">
              ...
              <div className="p-4 border-b border-gray-800/30">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-base font-semibold text-white">Course Content</h2>
                ...
                <div className="text-xs text-gray-400">
                  {course.chapters.length} chapters • {getAllModules().length} lessons • {formatDuration(course.stats?.duration || 0)}
                  {failedChapters.size > 0 && (
                    <div className="text-yellow-400 mt-1">
                      ⚠️ {failedChapters.size} chapter{failedChapters.size > 1 ? 's' : ''} failed to load modules
                    </div>
                  )}
                </div>
              </div>
                        ...
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="text-left flex-1">
                            ...
                            <div className="text-xs text-gray-400 mt-0.5">
                              {chapter.modules?.length || 0} lessons • {Math.round(chapterProgress)}% complete
                              {failedChapters.has(chapter.id) && (
                                <span className="text-yellow-400 ml-1">(modules failed to load)</span>
                              )}
                            </div>
                          </div>
                        </div>
                              ...
                              <div className="flex items-start gap-3">
                                <div className="flex-shrink-0 mt-0.5">
                                  {module.completed ? (
                                    ...
                                    <PlayCircle size={14} className={selectedModule?.id === module.id ? 'text-blue-400' : 'text-blue-300'} />
                                  ) : (
                                    <FileText size={14} className={selectedModule?.id === module.id ? 'text-green-400' : 'text-green-300'} />
                                  )}
                                </div>
                                
                                <div className="flex-1 min-w-0">
                                  <p className={`text-sm font-medium line-clamp-2 ${
                                    selectedModule?.id === module.id ? 'text-blue-400' : 'text-white group-hover:text-blue-300'
                                  }`}>
        ...
        <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-300">Duration: {formatDuration(course.stats?.duration || 0)}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Users size={14} className="text-purple-400" />
                        <span className="text-sm text-gray-300">Students: {course.stats?.enrollmentCount || 'N/A'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Globe size={14} className="text-yellow-400" />
                        <span className="text-sm text-gray-300">Language: {course.language || 'English'}</span>
                      </div>
                      {course.lastUpdated && (
                        <div className="flex items-center gap-2">
                          <Calendar size={14} className="text-orange-400" />
                          <span className="text-sm text-gray-300">Updated: {course.lastUpdated}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {course.requirements && course.requirements.length > 0 && (
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2">
                        {course.requirements.slice(0, 4).map((requirement, index) => (
                          <li key={index} className="flex items-start gap-2">
          ...
          <div 
            ...
            style={{ 
              background: 'linear-gradient(152.97deg, rgba(6,128,255,0.03) 18.75%, rgba(0,0,0,0.1) 100%)',
              ...
              <div className="max-w-5xl mx-auto p-4 sm:p-6 relative z-10">
                {contentLoading ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-400 text-sm">Loading lesson content...</p>
                  </div>
                ) : (
                  <div className="space-y-6">
                    {/* Video Player */}
                    {selectedModule.type === 'video' && moduleContent?.videoUrl && (
                      <div className="aspect-video bg-black rounded-xl overflow-hidden shadow-2xl border border-gray-700/30">
                        <CustomVideoPlayer
                          ...
                          onEnded={() => markModuleCompleted(selectedModule.id)}
                          documents={moduleContent?.attachments?.map((att: { id: string; originalName: string; url: string; type?: string; size: number }) => ({
                            id: att.id,
                            title: att.originalName,
                            url: att.url,
                            type: (att.type as 'pdf' | 'doc' | 'docx' | 'ppt' | 'pptx' | 'txt' | 'other') || 'other',
                            size: att.size ? `${(att.size / 1024 / 1024).toFixed(2)} MB` : undefined
                          }))}
                          className="w-full h-full"
                        />
                      </div>
                    ...
                    {moduleContent?.attachments && moduleContent.attachments.length > 0 && (
                      ...
                      >
                        ...
                        <div className="grid gap-3">
                          {moduleContent.attachments.map((attachment: { id: string; originalName: string; size: number; url: string; type?: string }) => (
                            <div 
                              key={attachment.id} 
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-white font-medium text-sm">{attachment.originalName}</p>
                    ...
                    <div className="flex justify-between items-center pt-6">
                        ...
                        {getPreviousModule() && (
                          ...
                          >
                            ...
                            <div className="text-left">
                              <div className="text-xs text-gray-400">Previous Lesson</div>
                      ...
                      <div className="pt-6">
                        {getNextModule() && (
                          ...
                          >
                            <div className="text-right">
                              <div className="text-xs text-blue-200">Next Lesson</div>
                              <div className="font-medium line-clamp-1 text-sm">
                                {getNextModule()?.title}
                              </div>
                            </div>
                ...
                <div className="text-center max-w-md mx-auto p-6">
                  ...
                  >
                    <div className="relative mb-6">
                      ...
                      <PlayCircle className="mx-auto text-blue-400 relative z-10" size={48} />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-3">Welcome to Your Course!</h3>
                    <p className="text-gray-400 mb-6 text-sm">
                      Ready to start learning? Select a lesson from the course content sidebar to begin your journey.
                    </p>
                    <button
                      onClick={() => setSidebarOpen(true)}
