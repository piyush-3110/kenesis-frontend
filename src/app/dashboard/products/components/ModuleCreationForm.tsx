...
const ModuleCreationForm: React.FC = () => {
  ...
  const handleFileUpload = (file: File, isMainFile = true) => {
    ...
    if (isMainFile) {
      ...
      if (file.size > limits.maxSize) {
        setErrors(prev => ({ 
          ...prev, 
          mainFile: `File size exceeds ${limits.maxSize / (1024 * 1024)}MB limit` 
        }));
        return;
      }
      ...
      if (file.size > attachmentLimits.maxSize) {
        setErrors(prev => ({ 
          ...prev, 
          attachments: `Attachment file size exceeds ${attachmentLimits.maxSize / (1024 * 1024)}MB limit` 
        }));
        return;
      }
  ...
  const validateForm = () => {
    ...
    if (!editingModuleId && !mainFile) {
      newErrors.mainFile = 'Main file is recommended for better learning experience';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedChapter = currentCourse.chapters.find(ch => ch.id === selectedChapterId);
    if (!selectedChapter) {
      console.error('❌ Selected chapter not found');
      addToast({ type: 'error', message: 'Selected chapter not found. Please select a valid chapter.' });
      return;
    }
    ...
    if (editingModuleId) {
      ...
      try {
        ...
        console.log('📤 API URL will be: /api/courses/' + currentCourse.id + '/modules');
        ...
        if (result.success && result.data) {
          ...
          if (result.isNotFound) {
            addToast({ type: 'error', message: 'Course or chapter not found. Please ensure the chapter was saved properly.' });
            return;
          }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (mainFile) {
                          ...
                          addToast({ 
                            type: 'info', 
                            message: 'Main file cleared due to module type change. Please upload a new file.' 
                          });
                        }
                      }}
                      className={inputClass}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative z-10">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Leave as 0 if duration is unknown</p>
                </div>

                <div className="flex items-end relative z-10">
                  <div className="w-full">
                    <label className="block text-white font-medium mb-3">Module Access</label>
                    <label className="flex items-center gap-3 cursor-pointer p-3 bg-gray-800/30 rounded-lg border border-gray-600 hover:border-gray-500 transition-colors">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (isChecked) {
                            addToast({ 
                              type: 'info', 
                              message: '💡 Preview modules can be accessed by anyone without purchasing the course. Great for attracting students!' 
                            });
                          }
                        }}
                      ...
                      />
                      <div>
                        <span className="text-white font-medium block">Available as Preview</span>
                        <span className="text-xs text-gray-400">Free to watch for everyone</span>
                      </div>
                    </label>
                  </div>
                </div>
                      ...
                      <label className="cursor-pointer w-full h-full flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-gray-500 text-xs mt-1">
                            {formData.type === 'video' && 'Max 500MB, MP4/WEBM/OGG/AVI/MOV/WMV/QuickTime'}
                            {formData.type === 'document' && 'Max 500MB, PDF/DOC/DOCX/PPT/PPTX'}
                          </p>
                        </div>
                        <input
                          type="file"
              ...
              <div>
                <label className="block text-white font-medium mb-3">
                  ...
                  <span className="text-xs text-gray-400 ml-2">Max 10 files, 500MB each</span>
                </label>
                <div className={cn(gradientBorderClass, "min-h-[100px]")}>
                  <div className="bg-[#010519] rounded-lg p-4">
                      ...
                      <div className="mb-4 space-y-2">
                        {attachmentFiles.map((file, index) => (
                          <div key={index} className="flex items-center justify-between bg-gray-800/50 rounded p-2">
                            <span className="text-white text-sm truncate flex-1 mr-2">{file.name}</span>
                            <button
                              type="button"
                    ...
                    {attachmentFiles.length < FILE_UPLOAD_LIMITS.moduleAttachment.maxCount && (
                      <label className="cursor-pointer block">
                        <div className="text-center py-4 border-2 border-dashed border-gray-600 rounded-lg hover:border-gray-500 transition-colors">
                          ...
                          <p className="text-gray-400 text-sm">Click to add attachment files</p>
                          <p className="text-gray-500 text-xs mt-1">
                    ...
                    {attachmentFiles.length >= FILE_UPLOAD_LIMITS.moduleAttachment.maxCount && (
                      <div className="text-center py-4">
                        <p className="text-yellow-400 text-sm">
                          Maximum {FILE_UPLOAD_LIMITS.moduleAttachment.maxCount} attachments reached
                        </p>
                      </div>
                    )}
                  </div>
                </div>
                {errors.attachments && <p className="text-red-400 text-sm mt-2">{errors.attachments}</p>}
              </div>
