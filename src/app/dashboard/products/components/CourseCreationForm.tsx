...
import { COURSE_TYPES, COURSE_LEVELS, SUPPORTED_LANGUAGES, PAYMENT_TOKENS, FILE_UPLOAD_LIMITS } from '../constants';
...
const CourseCreationForm: React.FC = () => {
  ...
  const uniqueChains = Array.from(new Set(PAYMENT_TOKENS.map(token => token.chainId)))
    .map(chainId => {
      const token = PAYMENT_TOKENS.find(t => t.chainId === chainId);
      return token ? { chainId: token.chainId, chainName: token.chainName } : null;
    })
    .filter(Boolean) as Array<{ chainId: number; chainName: string }>;

  // Filter tokens by selected chain
  const availableTokens = PAYMENT_TOKENS.filter(token => token.chainId === selectedChainId);

  const handleInputChange = (field: keyof CourseFormData, value: string | string[] | number) => {
  ...
  const handleFileUpload = (field: 'thumbnail' | 'previewVideo', file: File) => {
    const limits = field === 'thumbnail' ? FILE_UPLOAD_LIMITS.thumbnail : FILE_UPLOAD_LIMITS.previewVideo;
  ...
  const handleTokenToggle = (token: PaymentToken) => {
    setFormData(prev => ({
      ...prev,
      tokenToPayWith: prev.tokenToPayWith.some(t => t.address === token.address && t.chainId === token.chainId)
        ? prev.tokenToPayWith.filter(t => !(t.address === token.address && t.chainId === token.chainId))
  ...
  const validateForm = () => {
    ...
    if (formData.price > 0 && formData.tokenToPayWith.length === 0) newErrors.tokenToPayWith = 'Select at least one payment token for paid courses';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasAuth = TokenManager.hasTokens();
    if (!hasAuth) {
      addToast({ type: 'error', message: 'Authentication required. Please log in.' });
      logout();
    ...
    if (!validateForm()) {
      addToast({ type: 'error', message: 'Please fix validation errors before submitting' });
      return;
    }
    ...
    try {
      ...
      courseFormData.append('tokenToPayWith', JSON.stringify(formData.tokenToPayWith));
      courseFormData.append('accessDuration', formData.accessDuration.toString());
      courseFormData.append('affiliatePercentage', formData.affiliatePercentage.toString());
      courseFormData.append('availableQuantity', formData.availableQuantity.toString());
      ...
      if (result.success && result.data) {
        ...
        if (!updatedCourse.id) {
          ...
          addToast({ type: 'error', message: 'Course created but ID not found. Please try again.' });
          return;
        }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className={gradientBorderClass}>
              ...
              >
                {COURSE_LEVELS.map(level => (
                  <option key={level.value} value={level.value} className="bg-[#010519] text-white">
                    {level.label}
                  </option>
                ))}
              </select>
            ...
            <div className={gradientBorderClass}>
              ...
              >
                {SUPPORTED_LANGUAGES.map(language => (
                  <option key={language.value} value={language.value} className="bg-[#010519] text-white">
                    {language.label}
                  </option>
                ))}
              </select>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Access Duration */}
            <div>
              <label className="block text-white font-medium mb-3">Access Duration *</label>
              <div className={gradientBorderClass}>
                <select
                  value={formData.accessDuration}
                  onChange={(e) => handleInputChange('accessDuration', parseInt(e.target.value))}
                  className={inputClass}
                >
                  <option value={-1} className="bg-[#010519] text-white">Unlimited</option>
                  <option value={2592000} className="bg-[#010519] text-white">30 Days</option>
                  <option value={7776000} className="bg-[#010519] text-white">90 Days</option>
                  <option value={15552000} className="bg-[#010519] text-white">180 Days</option>
                  <option value={31536000} className="bg-[#010519] text-white">1 Year</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <label className="block text-white font-medium mb-3">Affiliate Commission (%) *</label>
              <div className={gradientBorderClass}>
                <input
                  type="number"
                  value={formData.affiliatePercentage}
                  onChange={(e) => handleInputChange('affiliatePercentage', parseFloat(e.target.value) || 0)}
                  placeholder="10"
                  min="0"
                  max="50"
                  step="0.1"
              ...
              {errors.affiliatePercentage && <p className="text-red-400 text-sm mt-2">{errors.affiliatePercentage}</p>}
            </div>

            {/* Available Quantity */}
            <div>
              <label className="block text-white font-medium mb-3">Max Enrollments *</label>
              <div className={gradientBorderClass}>
                <select
                ...
                >
                  <option value={-1} className="bg-[#010519] text-white">Unlimited</option>
                  <option value={50} className="bg-[#010519] text-white">50 Students</option>
                  <option value={100} className="bg-[#010519] text-white">100 Students</option>
                  <option value={250} className="bg-[#010519] text-white">250 Students</option>
                  <option value={500} className="bg-[#010519] text-white">500 Students</option>
                  <option value={1000} className="bg-[#010519] text-white">1000 Students</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          {/* Chain Selection */}
          <div>
            <label className="block text-white font-medium mb-3">Blockchain Network *</label>
            <div className={gradientBorderClass}>
              <select
              ...
              >
                {uniqueChains.map(chain => (
                  <option key={chain.chainId} value={chain.chainId} className="bg-[#010519] text-white">
                    {chain.chainName}
                  </option>
                ))}
              </select>
          ...
          <div>
            <label className="block text-white font-medium mb-3">
              ...
              <span className="text-sm text-gray-400 ml-2">(for {uniqueChains.find(c => c.chainId === selectedChainId)?.chainName})</span>
            </label>
            <div className="space-y-2">
              {availableTokens.map(token => {
                const isSelected = formData.tokenToPayWith.some(t => t.address === token.address && t.chainId === token.chainId);
                return (
                  <button
            ...
            {errors.tokenToPayWith && <p className="text-red-400 text-sm mt-2">{errors.tokenToPayWith}</p>}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <h3 className="text-lg font-semibold text-white">Course Details (Optional)</h3>
          
          {/* Requirements */}
          <div>
            <label className="block text-white font-medium mb-3">Prerequisites</label>
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2">
                  <div className={gradientBorderClass + " flex-1"}>
                    <input
                      ...
                      value={req}
                      onChange={(e) => {
                        const newReqs = [...(formData.metadata?.requirements || [])];
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const newReqs = formData.metadata?.requirements?.filter((_, i) => i !== index) || [];
                      setFormData(prev => ({
                        ...prev,
                        metadata: { ...prev.metadata, requirements: newReqs }
                      }));
          ...
          <div>
            ...
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2">
                  <div className={gradientBorderClass + " flex-1"}>
                    <input
                      ...
                      value={outcome}
                      onChange={(e) => {
                        const newOutcomes = [...(formData.metadata?.learningOutcomes || [])];
                        newOutcomes[index] = e.target.value;
                        setFormData(prev => ({
                          ...prev,
                          metadata: { ...prev.metadata, learningOutcomes: newOutcomes }
                        }));
                        // Clear error when user starts typing
                        if (errors.learningOutcomes) {
                          setErrors(prev => ({ ...prev, learningOutcomes: '' }));
                        }
                      }}
                      placeholder={`Learning outcome ${index + 1} (e.g., Build responsive web applications)`}
                      className={inputClass}
                      required
                    />
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      const newOutcomes = formData.metadata?.learningOutcomes?.filter((_, i) => i !== index) || [];
                      setFormData(prev => ({
                        ...prev,
                        metadata: { ...prev.metadata, learningOutcomes: newOutcomes }
                      }));
                    ...
                    disabled={(formData.metadata?.learningOutcomes?.length || 0) <= 3}
                    title={(formData.metadata?.learningOutcomes?.length || 0) <= 3 ? "Minimum 3 learning outcomes required" : "Remove this outcome"}
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
              ...
              {errors.learningOutcomes && <p className="text-red-400 text-sm">{errors.learningOutcomes}</p>}
          ...
          {/* Target Audience */}
          <div>
            <label className="block text-white font-medium mb-3">Target Audience</label>
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2">
                  <div className={gradientBorderClass + " flex-1"}>
                    <input
                      ...
                      value={audience}
                      onChange={(e) => {
                        const newAudience = [...(formData.metadata?.targetAudience || [])];
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const newAudience = formData.metadata?.targetAudience?.filter((_, i) => i !== index) || [];
                      setFormData(prev => ({
                        ...prev,
                        metadata: { ...prev.metadata, targetAudience: newAudience }
                      }));
