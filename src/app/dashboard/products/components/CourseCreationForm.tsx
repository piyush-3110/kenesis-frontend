...
const CourseCreationForm: React.FC = () => {
  ...
  const { currentCourse, updateCourse, setCurrentStep, setCurrentCourse } =
  ...
  const validateForm = () => {
    ...
    if (formData.tokensToPayWith.length === 0)
      newErrors.tokensToPayWith = "Select at least one payment token";
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.isForbidden) {
          addToast({
            type: "error",
            message: "Your instructor account is deactivated.",
          });
          return;
        }
        ...
        if (result.isNotFound) {
          addToast({
            type: "error",
            message: "Instructor not found. Please contact support.",
          });
          return;
        }
