        ...
        {primaryStatCards.map((stat) => {
          ...
          const getTrendIndicator = (trend: string) => {
            switch (trend) {
              ...
              case "medium": return <Activity className="w-3 h-3 text-yellow-400" />;
              case "low": return <TrendingUp className="w-3 h-3 text-green-400" />;
              case "active": return <Zap className="w-3 h-3 text-blue-400" />;
              case "success": return <Award className="w-3 h-3 text-green-400" />;
              case "caution": return <AlertCircle className="w-3 h-3 text-orange-400" />;
              default: return null;
            }
          };
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">Pending reviews by age - prioritize oldest submissions</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              >
                ...
                <div className={`text-xs px-2 py-1 rounded-full inline-block ${item.bgColor} ${item.color}`}>
                  {item.priority}
                </div>
              </div>
            ))}
              ...
              {totalWorkload > 0 && (
                <div className="h-full flex">
                  <div 
                    ...
                    style={{ width: `${((stats?.pendingBreakdown?.lessThan24h || 0) / totalWorkload) * 100}%` }}
                  />
                  <div 
                    className="bg-yellow-400" 
                    style={{ width: `${((stats?.pendingBreakdown?.oneToThreeDays || 0) / totalWorkload) * 100}%` }}
                  />
                  <div 
                    className="bg-orange-400" 
                    style={{ width: `${((stats?.pendingBreakdown?.threeTosevenDays || 0) / totalWorkload) * 100}%` }}
                  />
                  <div 
                    className="bg-red-400" 
                    style={{ width: `${((stats?.pendingBreakdown?.moreThanSevenDays || 0) / totalWorkload) * 100}%` }}
                  />
                </div>
              )}
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-gray-400 text-sm">
              Last {stats.recentActivity.length} actions
            </div>
          </div>
            ...
            {stats.recentActivity.slice(0, 10).map((activity, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-lg ${activity.action === 'approved' ? 'bg-green-400/10' : 'bg-red-400/10'}`}>
                  ...
                  <div className="flex-1">
                    <p className="text-white text-sm font-medium line-clamp-1">{activity.courseTitle}</p>
                    <p className="text-gray-400 text-xs flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${
                        activity.action === 'approved' ? 'bg-green-400/20 text-green-400' : 'bg-red-400/20 text-red-400'
                      }`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-red-300 text-sm">
                {criticalPending} courses have been pending for more than 7 days. Immediate attention required.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-yellow-300 text-sm">
                Average review time is {avgReviewTime.toFixed(1)} hours. Consider optimizing the review process.
              </p>
            </div>
          </div>
        </div>
