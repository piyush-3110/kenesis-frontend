...
import type { CourseForReview, PendingReviewItem, CourseApprovalData, CourseRejectionData } from "../api/approvalsApi";
...
const CourseReviewCard: React.FC<CourseReviewCardProps> = ({ 
  ...
  const isPendingReviewItem = (item: any): item is PendingReviewItem => {
    return item && typeof item === 'object' && 'course' in item && 'status' in item && 'submittedAt' in item;
  };
  ...
  const getInstructorName = () => {
    ...
    return instructor.username || 
           ...
           ((course as any)?.createdBy && typeof (course as any).createdBy === 'object' ? 
             (course as any).createdBy.username || (course as any).createdBy.email?.split('@')[0] : null) ||
           'Unknown Instructor';
  };
  ...
  const getInstructorEmail = () => {
    ...
    return instructor.email || 
           ((course as any)?.createdBy && typeof (course as any).createdBy === 'object' ? 
             (course as any).createdBy.email : null);
  };
  ...
  const getInstructorId = () => {
    ...
    return instructor.id || 
           instructor._id || 
           ((course as any)?.createdBy && typeof (course as any).createdBy === 'object' ? 
             (course as any).createdBy.id || (course as any).createdBy._id : null);
  };

  // Debug logging for instructor data (only in development)
  if (process.env.NODE_ENV === 'development') {
    console.log('🧑‍🏫 [APPROVAL] Course instructor data:', course?.instructor);
    console.log('🧑‍🏫 [APPROVAL] Instructor name:', getInstructorName());
    console.log('🧑‍🏫 [APPROVAL] Instructor email:', getInstructorEmail());
  }
  ...
  const handleNavigateToLearn = () => {
    try {
      console.log('🚀 [APPROVAL] Navigating to learn page for course:', courseId);
    ...
    >
      ...
      <div className="relative z-10">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-6 text-sm text-gray-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{formatDate(course?.createdAt || pendingInfo?.submittedAt || '')}</span>
                <span className="text-orange-400">
                  ({getDaysAgo(course?.createdAt || pendingInfo?.submittedAt || '')} days ago)
                </span>
              </div>
              {course?.price && (
                <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="text-right">
              ...
              <div className="text-sm font-medium text-orange-400 capitalize">
                {pendingInfo?.status || course?.latestHistoryEntry?.status || course?.status || 'Unknown'}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-300">
            ...
            <span>
              {course?.latestHistoryEntry?.message || 
               (pendingInfo ? `Submitted for review ${pendingInfo.daysPending} day${pendingInfo.daysPending !== 1 ? 's' : ''} ago` : 'No update message')}
            </span>
            <span className="text-gray-500">
              • {formatDate(course?.latestHistoryEntry?.timestamp || pendingInfo?.submittedAt || '')}
            </span>
            {course?.latestHistoryEntry?.metadata?.submissionCount && (
              <span className="text-orange-400">
                • Submission #{course.latestHistoryEntry.metadata.submissionCount}
              </span>
            )}
            {pendingInfo && (
            ...
            <div className="space-y-4 p-4 bg-gray-800/30 rounded-lg">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {course.chapters.map((chapter: any, index: number) => {
                      ...
                      const chapterKey = chapter._id || `chapter-${courseId}-${index}`;
                      
                      return (
                        <div key={chapterKey} className="bg-gray-700/30 rounded p-3">
                          <div className="text-sm font-medium text-blue-300 mb-1">
                            Chapter {index + 1}: {chapter.title || 'Untitled Chapter'}
                          </div>
                          <div className="text-xs text-gray-400">
                            {chapter.modules?.length || 0} module{(chapter.modules?.length || 0) !== 1 ? 's' : ''}
                          </div>
                          {chapter.modules && Array.isArray(chapter.modules) && chapter.modules.length > 0 && (
                            <div className="mt-2 space-y-1">
                              {chapter.modules.map((module: any, moduleIndex: number) => {
                                ...
                                const moduleKey = module._id || `module-${chapterKey}-${moduleIndex}`;
                                
                                return (
                                  <div key={moduleKey} className="flex items-center gap-2 text-xs text-gray-400">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {course.accessDuration && (
                  <div>
                    <h4 className="text-sm font-medium text-white mb-1">Access Duration</h4>
                    <div className="flex items-center gap-2 text-sm text-gray-300">
                ...
                <div>
                  <h4 className="text-sm font-medium text-white mb-1">Instructor</h4>
                  <div className="text-sm text-gray-300">
                    <div className="font-medium">{getInstructorName()}</div>
                    {getInstructorEmail() && (
                      <div className="text-xs text-gray-400 mt-1">{getInstructorEmail()}</div>
                    )}
                    {getInstructorId() && (
                      <div className="text-xs text-gray-500 mt-1">ID: {getInstructorId()}</div>
                    )}
                  </div>
                </div>
              </div>
        ...
        {showApprovalForm && (
          ...
          >
            <div className="bg-gray-900 rounded-xl p-6 max-w-lg w-full border border-gray-700 max-h-[85vh] overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-white">Approve Course</h3>
                <button
              ...
              <div className="flex items-center gap-3 mt-6">
                <button
                  ...
                  disabled={approveMutation.isPending || !approvalMessage.trim()}
        ...
        {showRejectionForm && (
          ...
          >
            <div className="bg-gray-900 rounded-xl p-6 max-w-lg w-full border border-gray-700 max-h-[85vh] overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-white">Reject Course</h3>
                <button
