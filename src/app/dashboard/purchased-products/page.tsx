...
import { Clock, Play, Star, Calendar, Shield, AlertCircle, ShoppingBag, Book } from 'lucide-react';
...
const PurchasedProductsPage: React.FC = () => {
  ...
  const loadPurchases = async () => {
    try {
      ...
      const requestParams = {
        status: (filter === 'all' ? 'all' : filter) as 'all' | 'active' | 'expired',
        limit: 50
      };

      console.log('📚 [PURCHASES] Starting request...');
      console.log('📚 [PURCHASES] Request params:', JSON.stringify(requestParams, null, 2));
      ...
      console.log('📚 [PURCHASES] Raw response received:', JSON.stringify(response, null, 2));
      console.log('📚 [PURCHASES] Response success:', response.success);
      console.log('📚 [PURCHASES] Response message:', response.message);
      console.log('📚 [PURCHASES] Response data:', JSON.stringify(response.data, null, 2));

      if (response.success) {
        // Handle nested response structure: response.data.purchases.purchases
        const purchasesData = response.data?.purchases?.purchases || response.data?.purchases || [];
        console.log('✅ [PURCHASES] Purchases extracted from response:', JSON.stringify(purchasesData, null, 2));
        console.log('✅ [PURCHASES] Number of purchases:', purchasesData.length);
        
        if (purchasesData.length === 0) {
          console.log('⚠️ [PURCHASES] No purchases found in response');
        } else {
          console.log('✅ [PURCHASES] First purchase sample:', JSON.stringify(purchasesData[0], null, 2));
        }
      ...
      setLoading(false);
      console.log('📚 [PURCHASES] Request completed. Loading state set to false.');
    }
  };
  ...
  const filteredPurchases = useMemo(() => {
    ...
    console.log('🔍 [FILTER] Raw purchases array:', JSON.stringify(purchases, null, 2));
    console.log('🔍 [FILTER] Purchases is array:', Array.isArray(purchases));
    console.log('🔍 [FILTER] Purchases length:', purchases?.length);
    
    if (!Array.isArray(purchases)) {
      console.log('⚠️ [FILTER] Purchases is not an array, returning empty array');
      return [];
    }
    ...
    switch (filter) {
      case 'active':
        filtered = purchases.filter(purchase => purchase.isActive && purchase.hasAccess);
        console.log('🔍 [FILTER] Active filter applied, result count:', filtered.length);
        break;
      case 'expired':
        filtered = purchases.filter(purchase => !purchase.isActive || !purchase.hasAccess);
        console.log('🔍 [FILTER] Expired filter applied, result count:', filtered.length);
        break;
      case 'listed':
        filtered = [];
        console.log('🔍 [FILTER] Listed filter applied, result count:', filtered.length);
        break;
      default:
        filtered = purchases;
        console.log('🔍 [FILTER] All filter applied, result count:', filtered.length);
        break;
    }
  ...
  const filteredListedCourses = useMemo(() => {
    ...
    console.log('🔍 [FILTER] Current filter:', filter);
    console.log('🔍 [FILTER] Listed courses array:', JSON.stringify(listedCourses, null, 2));
    
    if (filter !== 'listed' || !Array.isArray(listedCourses)) {
      console.log('🔍 [FILTER] Not listed filter or courses not array, returning empty');
      return [];
    }
  ...
  console.log('🎨 [RENDER] Filtered purchases count:', filteredPurchases.length);
  console.log('🎨 [RENDER] Filtered listed courses count:', filteredListedCourses.length);
          ...
          <img
                ...
                <div className="absolute top-3 left-3">
                  <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                    ...
                    course.status === 'submitted' ? 'bg-yellow-500/20 text-yellow-400' :
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {course.type === 'video' && course.stats.duration
                          ...
                          : course.type === 'document' && course.stats.totalPages
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-white mb-2">
            {filter === 'all' ? 'No courses purchased yet' : `No ${filter} courses found`}
          </h3>
          <p className="text-gray-400 mb-4">
            {filter === 'all' 
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-white">
                      ${purchase.purchasePrice} {purchase.tokenInfo?.symbol || purchase.tokenUsed}
                    </span>
                  </div>
                  
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-400">Access:</span>
                    <span className={getStatusColor(purchase.isActive, purchase.remainingDays)}>
