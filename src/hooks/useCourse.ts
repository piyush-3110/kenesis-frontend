  ...
  const createCourse = useCallback(async (courseData: FormData) => {
    try {
      ...
      if (!response.success) {
        ...
        if (response.message?.includes('Invalid or expired token') || response.message?.includes('Access token required')) {
        ...
        if (response.errors && response.errors.some(err => err.field === 'account')) {
        ...
        if (response.errors && response.errors.some(err => err.field === 'user')) {
        ...
        if (response.errors && response.errors.some(err => err.field === 'title')) {
      ...
      return { success: true, data: response.data, message: 'Course created successfully!' };
    } catch (error) {
...
export const useCreateChapter = () => {
  ...
  const createChapter = useCallback(async (courseId: string, chapterData: { title: string; description: string }) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (!response.success) {
        ...
        if (response.message?.includes('Access denied')) {
          const message = 'You are not authorized to add chapters to this course.';
          setError(message);
      ...
      return { success: true, data: response.data, message: 'Chapter created successfully!' };
    } catch (error) {
...
export const useCreateModule = () => {
  ...
  const createModule = useCallback(async (courseId: string, moduleData: FormData) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (!response.success) {
        // Handle specific error scenarios according to task requirements
        if (response.retryAfter) {
          const message = `Too many requests. Try again in ${formatRetryAfter(response.retryAfter)}.`;
          setError(message);
      ...
      return { success: true, data: response.data, message: 'Module created successfully!' };
    } catch (error) {
...
export const useSubmitCourseForReview = () => {
  ...
  const submitForReview = useCallback(async (courseId: string, submissionNotes?: string) => {
    try {
      setLoading(true);
      setError(null);

      const response = await CourseAPI.submitForReview(courseId, submissionNotes);
      ...
      if (!response.success) {
        ...
        // Handle validation errors (400)
        if (response.errors && response.errors.length > 0) {
          if (response.errors.some(err => err.field === 'submissionNotes')) {
      ...
      return { success: true, data: response.data, message: 'Course submitted successfully!' };
    } catch (error) {
