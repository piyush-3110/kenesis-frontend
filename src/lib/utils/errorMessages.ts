...
const BLOCKCHAIN_ERROR_MAPPINGS: ErrorMapping[] = [
  // Insufficient funds errors
  {
    ...
    message: "You don't have enough tokens to complete this purchase. Please check your wallet balance.",
  },
  ...
  // User rejection
  {
    ...
    message: "Transaction was cancelled. Please try again to complete your purchase.",
  },
  ...
  // Token approval errors
  {
    ...
    message: "Token approval failed. Please approve the token spending and try again.",
  },
  
  // Contract execution errors
  {
    pattern: /execution reverted|revert|invalid|contract call failed/i,
    message: "Transaction failed due to a smart contract issue. Please ensure all requirements are met and try again.",
  },
  
  // RPC errors
  {
    pattern: /rpc|provider|infura|alchemy/i,
    message: "Blockchain network is currently unavailable. Please try again in a moment.",
  },
  
  // Wallet connection errors
  {
    pattern: /wallet|metamask|connector/i,
    message: "Wallet connection issue. Please ensure your wallet is connected and try again.",
  },
...
const API_ERROR_MAPPINGS: ErrorMapping[] = [
  ...
  // Course access errors
  {
    ...
    message: "The course you're trying to purchase is not available. Please try a different course.",
  },
 ...
 */
export function getErrorMessage(error: unknown, context: 'blockchain' | 'api' | 'general' = 'general'): string {
  if (!error) {
  ...
  if (error instanceof Error) {
    ...
    errorMessage = String(errorObj.message || errorObj.error || errorObj.reason) || JSON.stringify(error);
  } else {
  ...
  for (const mapping of mappings) {
    const isMatch = mapping.pattern instanceof RegExp 
