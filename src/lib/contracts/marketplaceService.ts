...
import { ERC20_ABI, KENESIS_MARKETPLACE_ABI, type PaymentAmount } from "./abi";
import {
    getChainConfig,
...
export interface ContractPurchaseParams {
    ...
    tokenAmount: bigint; // The calculated amount of tokens for the purchase
    ...
    const quote: PurchaseQuote | undefined =
        tokenConfig && paymentAmountQuery.data
            ? {
                  ...
                  marketplaceFeePercent: marketplaceFeeQuery.data ? Number(marketplaceFeeQuery.data) / 100 : 0,
 ...
 */
export const useTokenApproval = (tokenString: string, spenderAmount: bigint) => {
    const { address: userAddress } = useAccount();
    const { address: marketplaceAddress } = useMarketplaceContract();
    const tokenConfig = getTokenConfig(tokenString);
    const tokenAddress = getContractTokenAddress(tokenString);
    ...
    const allowanceQuery = useReadContract({
        ...
        query: {
            enabled: !!userAddress && !!marketplaceAddress && !!tokenAddress && !tokenConfig?.isNative,
        },
    });
    ...
    const { isSuccess: isApproved, isLoading: isWaitingApproval } = useWaitForTransactionReceipt({
        hash: approveHash,
    });
...
export const usePurchaseCourse = () => {
    ...
    const purchaseCourse = async (params: ContractPurchaseParams): Promise<PurchaseResult> => {
        try {
            ...
            if (!nftMetadataUri) {
                ...
                if (!nftResult.success || !nftResult.metadataUri) {
                    throw new Error(`Failed to generate NFT metadata: ${nftResult.error}`);
                }
            ...
            if (!authResponse.success || !authResponse.data) {
                throw new Error(`Failed to get purchase authorization: ${authResponse.message}`);
            }
            ...
            const affiliatePercentage = BigInt(contractParams.affiliatePercentage * 100); // Convert to basis points
    ...
    const handleBackendStatusCheck = async (
        ...
        try {
            ...
            if (statusResponse.success) {
                ...
                console.warn("⚠️ Purchase status check failed:", statusResponse.message);
            }
    ...
    const nftTokenId = useMemo(() => {
        ...
        try {
            ...
            const purchaseEvent = receipt.logs.find(log => {
                ...
                return (
                    log.address.toLowerCase() === marketplaceAddress?.toLowerCase() && log.topics[0] === "0x..." // TODO: Add actual event signature hash
                );
            });
...
export const usePurchaseValidation = (params: ContractPurchaseParams) => {
    ...
    if (params.affiliatePercentage && (params.affiliatePercentage < 0 || params.affiliatePercentage > 50)) {
 ...
 */
export const formatPaymentAmount = (amount: bigint, decimals: number, symbol: string): string => {
...
export const calculateCostBreakdown = (
    ...
    const marketplaceFee = (paymentAmount * BigInt(Math.floor(marketplaceFeePercent * 100))) / BigInt(10000);
    const affiliateCommission = (paymentAmount * BigInt(Math.floor(affiliatePercentage * 100))) / BigInt(10000);
