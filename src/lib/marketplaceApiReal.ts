 ...
 */
function transformCourseToProduct(course: any): Product {
  ...
  try {
    return {
      ...
      author: course.instructor?.username || course.instructor?.name || 'Unknown Author',
      price: course.pricing?.amount || course.price || 0,
      currency: course.pricing?.currency || course.currency || 'USD',
      rating: course.stats?.rating || course.rating || 0,
      totalRatings: course.stats?.reviewCount || course.totalReviews || course.reviewCount || 0,
      image: course.thumbnail || '/images/landing/product.png',
      category: course.category || 'General',
      type: course.type || 'video',
      createdAt: course.createdAt || new Date().toISOString(),
      isPurchased: course.isPurchased || false,
      purchaseDate: course.purchaseDate,
      accessLevel: course.accessLevel || 'preview',
      topics: course.topics || course.metadata?.tags || course.learningOutcomes || []
    };
  } catch (error) {
...
export async function fetchProducts(
  ...
  try {
    ...
    if (!response.success) {
      console.error('❌ Failed to fetch products from backend:', response.message);
...
export async function fetchCategories(): Promise<Category[]> {
  try {
    ...
    if (!response.success) {
      console.error('❌ Failed to fetch categories from backend:', response.message);
      return [];
    }
...
export async function fetchPriceRange(): Promise<PriceRange> {
  try {
    ...
    if (!response.success || !response.data?.courses) {
      console.error('❌ Failed to fetch price range from backend:', response.message);
      return { min: 0, max: 1000, currency: 'USD' };
    }

    const prices = response.data.courses.map((course: any) => course.price || 0);
    ...
    console.log('✅ Successfully calculated price range from backend:', priceRange);
    return priceRange;
...
export async function fetchSearchSuggestions(query: string): Promise<{ text: string; type: string }[]> {
  ...
  try {
    ...
    if (!response.success || !response.data?.courses) {
      console.error('❌ Failed to fetch search suggestions from backend:', response.message);
      return [];
    }
    ...
    response.data.courses.forEach((course: any) => {
      ...
      if (course.instructor?.username) suggestions.add(course.instructor.username);
    });
    ...
    console.log(`✅ Successfully fetched ${result.length} search suggestions from backend`);
    return result;
...
export async function searchProducts(query: string): Promise<Product[]> {
  ...
  try {
    ...
    if (!response.success || !response.data?.courses) {
      console.error('❌ Failed to search products from backend:', response.message);
      return [];
    }

    const products = response.data.courses.map(transformCourseToProduct);
    console.log(`✅ Successfully found ${products.length} products from backend search`);
...
export async function fetchProduct(id: string): Promise<Product | null> {
  try {
    ...
    if (!response.success) {
      console.error('❌ Failed to fetch product from backend:', response.message);
      return null;
    }
...
export async function fetchTypeStats(): Promise<{ video: number; document: number }> {
  try {
    ...
    const videoCount = videoResponse.success ? (videoResponse.data?.pagination?.totalCourses || 0) : 0;
    const documentCount = documentResponse.success ? (documentResponse.data?.pagination?.totalCourses || 0) : 0;
